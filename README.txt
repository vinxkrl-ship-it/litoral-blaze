...
export default function App() {
  ...
  const { rounds, online, refetch: refetchRounds } = useRounds()
  const { signals, createSignal, updateSignal, deleteSignal, resetSignals } = useSignals()
  // Fix 13: sessão admin expira em 8h
  const [adminSession, setAdminSession] = useState(() => {
    const expiry = localStorage.getItem('lb_admin_expiry')
    if (!expiry || Date.now() > parseInt(expiry)) {
      localStorage.removeItem('lb_admin')
      localStorage.removeItem('lb_admin_expiry')
      return false
    }
    return localStorage.getItem('lb_admin') === '1'
  })
  ...
  async function handleLogin(type, arg1, arg2, arg3) {
    if (type === 'admin') {
      ...
      localStorage.setItem('lb_admin_expiry', String(Date.now() + 8 * 60 * 60 * 1000))
