...
// Exporta detalhes completos de cada pilar para o painel admin
export function analyzeDebug(rounds) {
  ...
  const p1 = pillar1(today)
  const triggerHit = p1.numbers.find(x => x.num === lastNum)
  const p2 = triggerHit ? pillar2(today, lastNum) : { valid: false, avg: 0, consistency: 0 }
  const p3 = pillar3(today)
  const p4 = pillar4(today)
  const now = new Date()
  const estimateMin = p2.avg || 5
  ...
  return {
    totalRodadas: today.length,
    lastNum,
    p1: {
      active: p1.valid && !!triggerHit,
      desc: 'Gatilhos Frequentes',
      detail: p1.valid
        ? `Gatilhos: ${p1.numbers.map(x=>`#${x.num}(${x.count}x)`).join(', ')} | Último: #${lastNum} ${triggerHit ? '✅ MATCH' : '❌ sem match'}`
        : 'Sem padrão de gatilhos ainda',
    },
    p2: {
      active: p2.valid,
      desc: 'Intervalo Consistente',
      detail: p2.valid
        ? `Média: ${p2.avg} min após gatilho | Consistência: ${p2.consistency}%`
        : `Consistência insuficiente (${p2.consistency || 0}%) — mín 60%`,
    },
    p3: {
      active: isHotMin,
      desc: 'Minutos Quentes',
      detail: p3.minutes.length
        ? `Minutos com mais brancos: ${p3.minutes.slice(0,5).map(x=>`${String(x.min).padStart(2,'0')}min(${x.count}x)`).join(', ')} | Alvo: ${String(targetMin).padStart(2,'0')}min ${isHotMin ? '✅' : '❌'}`
        : 'Sem minutos quentes identificados',
    },
    p4: {
      active: p4.detected,
      desc: 'Ritmo de Brancos',
      detail: p4.detected
        ? `Gap médio entre brancos: ${p4.gap} min (ideal 7-15 min) ✅`
        : 'Gap entre brancos fora do intervalo ideal (7-15 min)',
    },
    p5: {
      active: p5.detected,
      desc: 'Confirmação Histórica',
      detail: p5.detected
        ? `Padrão confirmado na hora anterior (+${p5.boost}% confiança) ✅`
        : 'Sem confirmação histórica na hora anterior',
    },
    targetMin,
    estimateMin,
  }
