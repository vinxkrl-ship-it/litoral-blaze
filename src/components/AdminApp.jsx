...
export default function AdminApp({ rounds, online, signals, onLogout, onCreateSignal, onUpdateSignal, onDeleteSignal, onResetSignals }) {
  ...
  const debug = analyzeDebug(rounds)
      ...
      <div className="admin-wrap">
        ...
        <div style={{background:'var(--bg-2)',border:'1px solid var(--border)',borderRadius:'12px',padding:'1rem 1.2rem',marginBottom:'1rem'}}>
          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'1rem',fontWeight:700,fontSize:'.88rem'}}>
            ...
            RAIO-X DA IA — O QUE ELA ESTÁ PENSANDO
            <span style={{marginLeft:'auto',fontSize:'.72rem',padding:'3px 10px',borderRadius:'20px',background: aiResult.shouldSend ? 'rgba(34,197,94,.15)' : 'rgba(255,59,59,.1)', color: aiResult.shouldSend ? 'var(--green)' : 'var(--text-3)', border: `1px solid ${aiResult.shouldSend ? 'var(--green)' : 'var(--border)'}`, fontWeight:600}}>
              {aiResult.shouldSend ? `🔥 SINAL PRONTO — ${aiResult.time}` : aiResult.reason || 'Analisando...'}
            </span>
          </div>
          ...
          {!debug ? (
            ...
            <>
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'8px',marginBottom:'1rem'}}>
                {[debug.p1, debug.p2, debug.p3, debug.p4, debug.p5].map((p, i) => (
                  <div key={i} style={{
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'4px'}}>
                      <div style={{
                        ...
                        border: `1px solid ${p.active ? 'var(--green)' : 'var(--border)'}`
                      }}>{i+1}</div>
              ...
              <div style={{display:'flex',gap:'1rem',flexWrap:'wrap',fontSize:'.75rem',color:'var(--text-3)',borderTop:'1px solid var(--border)',paddingTop:'.7rem'}}>
                <span>📊 Rodadas hoje: <strong style={{color:'var(--text-1)'}}>{debug.totalRodadas}</strong></span>
                <span>🎯 Último número: <strong style={{color:'var(--text-1)'}}>#{debug.lastNum ?? '—'}</strong></span>
                <span>⏱ Estima próximo branco em: <strong style={{color:'var(--text-1)'}}>{debug.estimateMin} min</strong></span>
                <span>🕐 Minuto alvo: <strong style={{color:'var(--text-1)'}}>{String(debug.targetMin).padStart(2,'0')}min</strong></span>
