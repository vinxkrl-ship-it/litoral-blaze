...
export default function UserApp({ profile, rounds, online, signals, onLogout, onUpdateSignal }) {
  ...
  // Paywall para free após primeiro sinal resolvido
  useEffect(() => {
  ...
  // ✅ CORREÇÃO 4: Auto win/loss com janela ±1 minuto
  useEffect(() => {
    ...
    signals.forEach(async sig => {
      ...
      // Procura branco dentro da janela ±1min
      const white = rounds.find(r => {
        const t = new Date(r.time)
        return t >= startW && t <= endW && r.color === 'white'
      })
      if (white) {
        await onUpdateSignal(sig.id, { status: 'win', result_time: white.time })
        toast(`✅ WIN! Branco às ${white.display || white.time.slice(11,16)}`, 'success')
        return
      }
      // LOSS: janela passou E tem pelo menos 1 rodada depois
      const now = new Date()
      if (now > endW) {
 ...
 // IA com bloqueio de janela: só 1 sinal por vez, pausa durante janela ativa
  useEffect(() => {
    ...
    supabase.from('signals').insert({
      ...
      note: `${r.activeCount}/5 pilares · gatilho:${r.trigger}`,
      is_ai: true,
      status: 'active'
  ...
  return (
    <div className="app visible">
      ...
      <main className="main">
        ...
        {/* ✅ CORREÇÃO 1: Horário da última rodada usando display (BRT correto) */}
        {/* ✅ CORREÇÃO 3: Sem janela de countdown */}
        {actives.length === 0 ? (
        ...
        {/* MÉDIA DE BRANCOS POR HORÁRIO */}
        <div className="card" style={{marginBottom:'1rem'}}>
          <div className="card-h">
            <div className="card-t"><i className="fas fa-clock"></i> Média de Brancos por Horário</div>
          </div>
          <div className="card-b" style={{padding:'0.5rem 0.8rem'}}>
            {(() => {
              const byHour = {}
              rounds.forEach(r => {
                if (r.color === 'white') {
              ...
              if (entries.length === 0) return (
                <p style={{color:'var(--text-3)',textAlign:'center',padding:'1rem',fontSize:'.82rem'}}>Aguardando dados...</p>
              )
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                  {entries.map(([h, count]) => (
                    <div key={h} style={{display:'flex',alignItems:'center',gap:'8px'}}>
                      <span style={{width:'32px',fontSize:'.75rem',color:'var(--text-2)',fontWeight:600,flexShrink:0}}>
                      ...
                      <span style={{width:'20px',fontSize:'.75rem',color:'var(--text-1)',fontWeight:700,textAlign:'right',flexShrink:0}}>
                        {count}
                      </span>
                    </div>
                  ))}
